 ...
 */
export function resolvePhaseSpeed(horse, phase, ctx, effort = 1) {
  ...
  const kickReadiness = Math.max(0, Math.min(1, (effort - 0.4) / 0.55));
  const k = getKickBlend(phase, ctx) * kickReadiness;
